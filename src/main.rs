...
fn init_kbc_manager() -> Result<(), Box<dyn Error>> {
    let kbc_manager_mutex_clone = Arc::clone(&kbc_manager::KBC_MANAGER);
    let mut kbc_manager = match kbc_manager_mutex_clone.lock() {
        Ok(manager) => manager,
        Err(e) => {
            return Err(into_err(
                "Some error happend when init get kbc manager(Mutex)'s lock".to_string(),
            ));
        }
    };
    kbc_manager.init();
